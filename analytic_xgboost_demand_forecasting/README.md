...
The project is designed as a proof-of-concept demand forecasting framework. It demonstrates how macroeconomic, housing, borrower, geography, and funnel-related signals can be converted into an actionable market prioritization model.
...
- Weekly time periods
- Housing, borrower, macroeconomic, geography, and funnel-related features
...
- **Borrower profile:** median income, average credit score, average DTI
...
The project intentionally introduces missing values to make the synthetic dataset closer to a realistic business dataset, where macro, housing, borrower, and funnel-related variables may not always be fully available.
...
- **Numeric features with missingness at or above 3%** are filled using the median value within the same `state-submarket` group.
- If a `state-submarket` median is unavailable, the feature-level global median is used as a fallback.

This approach avoids unnecessary over-imputation for small missing rates while still stabilizing features with more material missingness. It also preserves local market structure by using state-submarket medians before falling back to a global median.
...
- Median-based handling for selected numeric missing values
...
A random stratified split is used because this project is framed as a synthetic cross-sectional classification demonstration, not a strict forward-time production forecasting system.

In a production setting, this framework should be extended with time-based or rolling-origin validation to test whether the model generalizes to future market and macroeconomic regimes.
...
```text
P(high demand = 1 | market, macro, housing, borrower, and funnel features)
```
...
Rather than treating demand as one national signal, the framework suggests that demand should be monitored at the market-segment level. This is especially relevant for a home equity investment product because homeowner equity, property values, borrower profiles, and qualification likelihood can vary significantly across regions and submarkets.
...
In production, this framework should be validated using real application, marketing, property, underwriting, and funding data. A production version should also include time-based validation, rolling-origin backtesting, calibration checks, and ongoing model monitoring.
